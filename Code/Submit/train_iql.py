...
from iql import IQL
from iql_experiment import IQLExperiment
from utils import DataLoader

# ipython ./train_iql.py -- -f "./results/run1" --cql_alpha=0.5 --cql_tau=1.0
...
def train(params, rng, loader_train, loader_validation):
    iql_net = IQL(state_dim=params["embed_state_dim"], nb_actions=params["num_actions"], gamma=params["gamma"], 
                learning_rate=params["rl_learning_rate"], update_freq=params["update_freq"], 
                tau=params.get("iql_tau", 0.005), expectile=params.get("iql_expectile", 0.8), 
                temperature=params.get("iql_temperature", 1.0), rng=rng, device=params["device"], 
                sided_Q=params["sided_Q"], network_size=params["rl_network_size"])
    
    print('Initializing IQL Experiment')
    expt = IQLExperiment(data_loader_train=loader_train, data_loader_validation=loader_validation, 
                       iql_network=iql_net, ps=0, ns=2,
                       folder_location=params["folder_location"], folder_name=params["folder_name"], 
    ...
    print('Running experiment (training IQL Networks)')
    expt.do_epochs(number=params["exp_num_epochs"])
    print("Training IQL Networks finished successfully")
...
@click.option('--iql_tau', default=0.005, help="Soft target update rate for IQL.")
@click.option('--iql_expectile', default=0.8, help="Expectile value for asymmetric value regression.")
@click.option('--iql_temperature', default=1.0, help="Temperature for advantage-weighted policy updates.")
def run(options, folder, save, iql_tau, iql_expectile, iql_temperature):
    '''
    Run to train IQL separately from SC-Network, or alternatively call make_train() from a script.
    '''
    folder = os.path.abspath(folder)
    with open(os.path.join(folder, "config.yaml")) as f:
        params = yaml.safe_load(f)

    params["iql_tau"] = iql_tau
    params["iql_expectile"] = iql_expectile
    params["iql_temperature"] = iql_temperature
