...
# ipython ./train_cql.py -- -f "./results/run1"
...
def train(params, rng, loader_train, loader_validation):
    # Initialize the CQL network instead of RL/DQN
    qnet = CQL(state_dim=params["embed_state_dim"], nb_actions=params["num_actions"], gamma=params["gamma"], 
               learning_rate=params["rl_learning_rate"], update_freq=params["update_freq"], use_ddqn=params["use_ddqn"], 
               rng=rng, device=params["device"], sided_Q=params["sided_Q"], network_size=params["rl_network_size"],
               cql_alpha=params.get("cql_alpha", 0.5), cql_tau=params.get("cql_tau", 1.0))
    
    print('Initializing Experiment')
    # Use the existing DQNExperiment class but with a new folder_name to prevent overwriting
    ...
    print('Running experiment (training CQL Networks)')
    expt.do_epochs(number=params["exp_num_epochs"])
    print("Training CQL Networks finished successfully")
...
@click.option('--cql_alpha', default=0.5, help="CQL alpha parameter (regularization strength)")
@click.option('--cql_tau', default=1.0, help="CQL temperature parameter")
def run(options, folder, save, cql_alpha, cql_tau):
    folder = os.path.abspath(folder)
    with open(os.path.join(folder, "config.yaml")) as f:
        params = yaml.safe_load(f)
    
    params["cql_alpha"] = cql_alpha
    params["cql_tau"] = cql_tau
